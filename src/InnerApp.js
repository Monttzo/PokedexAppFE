...
class InnerApp extends Component {
    ...
    componentDidUpdate(prevProps) {
        ...
        if(prevProps.isFavs !== this.props.isFavs || prevProps.user !== this.props.user || prevProps.token !== this.props.token){
            if(this.props.token !== null){
                ...
                axios.get('http://127.0.0.1:8000/api/auth/favorite/myFavs/',{headers: {Authorization: `Bearer ${token}`}}).then((response) => {
    ...
    addFav = (pokemon) => {
        ...
        axios.post('http://127.0.0.1:8000/api/auth/favorite/create/', fav).then(async (response) => {
    ...
    removeFav = (favorite) => {
        ...
        axios.delete(`http://127.0.0.1:8000/api/auth/favorite/delete/${favid}`).then(async (response) => {
            let token = this.state.token;
            axios.get('http://127.0.0.1:8000/api/auth/favorite/myFavs/',{headers: {Authorization: `Bearer ${token}`}}).then((response) => {
                this.setState({favs: response.data.response});
                this.closeModalPokemon();
    ...
    loadFavs = () => {
        if(this.props.token !== null){
            let token = this.props.token; 
            axios.get('http://127.0.0.1:8000/api/auth/favorite/myFavs/',{headers: {Authorization: `Bearer ${token}`}}).then(async (response) => {
                this.setState({favs: response.data.response});
