...
class App extends Component {
  ...
  LogIn = () => {
    if(this.email.value !== '' && this.password.value !== ''){
      ...
      axios.post('http://127.0.0.1:8000/api/auth/login/', user).then(async (response) => {
        let accessToken = response.data.access_token;
        axios.post('http://127.0.0.1:8000/api/auth/me/',{}, {headers: {Authorization: `Bearer ${accessToken}`}}).then(async (response) => {
  ...
  Logout = () => {
    ...
    axios.post('http://127.0.0.1:8000/api/auth/logout/',{}, {headers: {Authorization: `Bearer ${accessToken}`}}).then(async (response) => {
  ...
  SingUp = () => {
    ...
    if(this.name.value !== '' && this.email.value !== '' && this.password.value !== '' && this.rpassword.value !== ''){
      ...
      axios.post('http://127.0.0.1:8000/api/auth/register/', user).then(async (response) => {
        let user = {
          email: this.email.value,
          password: this.password.value
        }
  ...
  saveProfile = () => {
    ...
    let accessToken = this.state.token;
    axios.put('http://127.0.0.1:8000/api/auth/update/', user, {headers: {Authorization: `Bearer ${accessToken}`}}).then(async (response) => {
        axios.post('http://127.0.0.1:8000/api/auth/me/',{}, {headers: {Authorization: `Bearer ${accessToken}`}}).then(async (response) => {
          let user = response.data;
